...
class PHPExcel_Reader_HTML extends PHPExcel_Reader_Abstract implements PHPExcel_Reader_IReader
{
    ...
    protected function flushCell($sheet, $column, $row, &$cellContent)
    {
        if (is_string($cellContent)) {
            ...
            if (trim($cellContent) > '') {
                        $strArr = explode(".", $cellContent);
                        $allContainNumeric = false;
                        foreach($strArr as $strPart)
                        {
                            $strArr2 = explode(",", $strPart);
                            foreach($strArr2 as $strPart2)
                            {
                                if(is_numeric($strPart2))
                                {
                                        $allContainNumeric = true;
                                        // echo $strPart . " is numeric<br />";
                                }
                                else 
                                {
                                        $allContainNumeric = false;
                                        break;
                                        // echo $strPart . " is not numeric<br />";
                                }
                            }
                        }
                        if($allContainNumeric)
                        {
                            $cellContent = str_replace("#",".",str_replace(".","",str_replace(",","#",$cellContent)));
                            //log_message('error',$domText);
                        }

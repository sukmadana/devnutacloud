<?php
include_once "IncomingStock/AddIncomingStock.php";
include_once "IncomingStock/ListIncomingStock.php";
include_once "IncomingStock/ViewIncomingStock.php";
include_once "IncomingStock/EditIncomingStock.php";
include_once "IncomingStock/DeleteIncomingStock.php";

class Stokmasuk extends MY_Controller

{
    use AddIncomingStock, ListIncomingStock, ViewIncomingStock, EditIncomingStock, DeleteIncomingStock;
    ...
    function __construct()
    {
        ...
        $this->data = [
            'page_part' => 'incoming-stock/index',
            'js_chart' => [],
    ...
    public function simpan()
    {
        ...
        if (!$storing)
            return redirect(base_url('stokmasuk/add?outlet=' . $this->input->post('outlet')));
...
//                return redirect($this->input->server('HTTP_REFERER'));
                redirect(base_url('stokmasuk/?outlet=' . $this->input->post('outlet') .
...
//              redirect(base_url('koreksistok/view/' . $this->input->post('outlet') . '/' . $this->input->post('transaction_id') ) );
        ...
        foreach ($this->input->post('item') as $key => $value) {
            ...
            $insert_query = $this->Stockopnamedetail->get_stockopnamedetail_rowarray($this->input->post('outlet'), $detail_id, $detail_devno);
                ...
                $this->Firebasemodel->push_firebase($this->input->post('outlet'), $last_insert_data,
                    $detail_id, $detail_devno, $this->perusahaanno, 0);
            }
        }
        return true;
    }
    ...
    protected function generateAttribute($cloudDevno)
    {
        ...
        $code = $this->db->query($this->Stockopname->get_generate_nomorstokmasuk(

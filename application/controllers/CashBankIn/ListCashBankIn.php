...
trait ListCashBankIn
{
    public function index()
    {
        ...
        $this->setData('page_part', 'webparts/uang-masuk/list');
        $this->addJsPart('webparts/uang-masuk/list_js');
        $this->setData('list_transaksi', $this->get_list_uang_masuk($this->data['selected_outlet'], $dateStart, $dateEnd));

        $this->load->view('main_part', $this->data);
    }

    protected function get_list_uang_masuk($outlet, $date_start, $date_end)
    {
...
(
		...
		c.AccountID, CONCAT(m.BankName,' ', m.AccountNumber,' ', m.AccountName) AS Rekening,
		 c.Amount, c.Note,c.ReceivedFrom, IncomeType, 1 as IsCloud, NULL as DeviceNo
		 FROM cloud_cashbankin c
		...
		SELECT TransactionID,TransactionNumber,TransactionDate,TransactionTime,
		c.AccountID, CONCAT(m.BankName,' ', m.AccountNumber,' ', m.AccountName) AS Rekening,
		 c.Amount, c.Note,c.ReceivedFrom, IncomeType, 0 as IsCloud, c.DeviceNo
		 FROM cashbankin c
		  INNER JOIN mastercashbankaccount m 
		  ON m.PerusahaanNo=c.PerusahaanNo
		  AND m.DeviceID=c.DeviceID

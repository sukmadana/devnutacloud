<?php
include_once "OutgoingStock/AddOutgoingStock.php";
include_once "OutgoingStock/ListOutgoingStock.php";
include_once "OutgoingStock/ViewOutgoingStock.php";
include_once "OutgoingStock/EditOutgoingStock.php";
include_once "OutgoingStock/DeleteOutgoingStock.php";

class Stokkeluar extends MY_Controller

{
    use AddOutgoingStock,ListOutgoingStock,ViewOutgoingStock,EditOutgoingStock,DeleteOutgoingStock;
    ...
    function __construct()
    {
        ...
        $this->data = [
            'page_part' 					=> 'outgoing-stock/index',
            'js_chart' 						=> [],
    ...
    public function simpan(){
        ...
        if (!$storing)
            return redirect(base_url('stokkeluar/add?outlet=' . $this->input->post('outlet')));
...
//                return redirect($this->input->server('HTTP_REFERER'));
                redirect(base_url('stokkeluar/?outlet=' . $this->input->post('outlet').
...
//              redirect(base_url('koreksistok/view/' . $this->input->post('outlet') . '/' . $this->input->post('transaction_id') ) );
    ...
    protected function storeDetail($cloudDevno, $firebase=true)
    {
        foreach ($this->input->post('item') as $key => $value) {
            ...
            if ($firebase) {
                $insert_query = $this->Stockopnamedetail->get_stockopnamedetail_rowarray($this->input->post('outlet'), $detail_id, $detail_devno);
                $last_insert_data =  array(
                    "table"     => 'stockopnamedetail',
                    "column"    => $insert_query
                );
                $this->load->model('Firebasemodel');
                $this->Firebasemodel->push_firebase($this->input->post('outlet'),$last_insert_data, $detail_id, $detail_devno, $this->perusahaanno, 0);
            }
        }
        return true;
    }
    ...
    protected function generateAttribute($cloudDevno)
    {
        ...
        $code = $this->db->query($this->Stockopname->get_generate_nomorstokkeluar(
    ...
    protected function addNewDetail($key, $cloudDevno)
    {
        ...
        if ($this->input->post('mode') == "edit") {
            ...
            $atribut['TransactionDeviceNo'] = $cloudDevno;
            $atribut['Varian'] = 'Nuta';

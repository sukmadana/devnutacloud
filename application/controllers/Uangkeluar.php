<?php
include_once "CashBankOut/AddCashBankOut.php";
include_once "CashBankOut/ListCashBankOut.php";
include_once "CashBankOut/ViewCashBankOut.php";
include_once "CashBankOut/EditCashBankOut.php";
include_once "CashBankOut/DeleteCashBankOut.php";

class Uangkeluar extends MY_Controller

{
    use AddCashBankOut,ListCashBankOut,ViewCashBankOut,EditCashBankOut,DeleteCashBankOut;
...
WHERE 
    ...
    protected function get_list_uangkeluar_by_id($outlet, $id, $iscloud = 1, $deviceno = -1)
    {
        $deviceClause = '';
        if ($iscloud === 1) {
            $tbl = 'cloud_cashbankout';
        } else {
            $tbl = 'cashbankout';
    ...
    protected function get_uangkeluar_by_id_rowarray($outlet, $id, $iscloud = 1)
    {
        $tbl = $iscloud === 1 ? "cloud_cashbankout" : "cashbankout";
        $query = "
        select * from $tbl 
            where DeviceID=? 
                and PerusahaanNo=?
...
//        echo $this->input->post('amount');
        ...
        if ($this->input->post('mode')=="new") {
            ...
            $storing = $this->Uangkeluarmodel->create($this->attributes);
            ...
            $storing = $this->Uangkeluarmodel->updateUangKeluar($iscloud, $clause, $this->attributes);
        }

        if (!$storing) {
//            var_dump($this->attributes);
//            var_dump($storing);
            return redirect(base_url('uangkeluar/add?outlet=' . $this->input->post('outlet')));
        }
        ...
        $insert_query = $this->get_uangkeluar_by_id_rowarray($this->input->post('outlet'), $tr_id, $iscloud);
        $last_insert_data =  array(
            "table"     => $iscloud === 1 ? 'cloudcashbankout' : 'cashbankout',
            "column"    => $insert_query
        );
        ...
        redirect(base_url('uangkeluar/?outlet=' . $this->input->post('outlet').
    ...
    protected function generateAttribute($cloudDevno)
    {
        ...
        if ($this->input->post('mode')=="edit") {
            return [
                ...
                'SpendingType' => $this->input->post('jenis'),
        ...
        $id = $this->db->query($this->Uangkeluarmodel->get_transaction_id($this->input->post('outlet')))->first_row();
        $code = $this->db->query($this->Uangkeluarmodel->get_generate_nomoruangkeluar(

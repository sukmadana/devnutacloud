...
trait DeleteCashBankOut
{
    public function destroy($dateStart, $dateEnd)
    {
        ...
        $whr = $this->getWhere();
        $this->Uangkeluarmodel->deleteUangKeluar($iscloud, $whr);

        $this->load->model('Firebasemodel');
        $deleted_data = array(
            "table" => $iscloud === 1 ? "deletecloudcashbankout" : 'deletecashbankout',
        ...
        return redirect(base_url('uangkeluar/?outlet=' . $this->getWhere()['DeviceID'] . '&ds=' . $dateStart . '&de=' . $dateEnd));
    }
    ...
    protected function runDeleteValidation()
    {
        ...
        ($this->form_validation->run()) ?: redirect('/uangkeluar/?outlet=' . $this->input->post('outlet_id'));

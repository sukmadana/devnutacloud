...
<script type="text/javascript">
    ...
    purchase = {
        ...
        bayarTunai: function() {
            var total = this.totalPayment();
            var bayarTunai = parseFloat($('#bayartunai').val());
            if (bayarTunai >= total) {
                var kembalian = bayarTunai - total;
        ...
        bayarKartu: function() {
            var total = this.totalPayment();
            var totalBelanja = $('#belanjakartu').val(total);
            var parseTotalBelanja = parseFloat($('#belanjakartu').val());
            var charge = parseFloat($('#chargekartu').val());
            var totalKartu = (parseTotalBelanja * charge / 100) + parseTotalBelanja;
            $('#totalkartu').val(totalKartu);
        },

        bayarCampuran: function() {
            var total = this.totalPayment();
            var totalBelanja = $('#belanjacampuran').val(total);
            var parseTotalBelanja = parseFloat($('#belanjacampuran').val());
            var tunai = parseFloat($('#tunaicampuran').val());
            var charge = parseFloat($('#chargecampuran').val());
            var sisaTotal = parseTotalBelanja - tunai;
            var hargaTotal = (sisaTotal * charge / 100) + sisaTotal;
            $('#totalcampuran').val(hargaTotal);
        }
    }
    ...
    $('.form-store').submit(function(e) {
        $('#btn-simpan-single-outlet').prop('disabled', true);
        ...
        if ($('input[name=datetime]').val() == '') {
            e.preventDefault();
            $('#btn-simpan-single-outlet').prop('disabled', false);
            return alert("Isian tanggal dibutuhkan");
        }

        if ($('.alertBank').val() == '') {
            e.preventDefault();
            $('#btn-simpan-single-outlet').prop('disabled', false);
            return alert("Form 'Masuk Ke' belum dipilih")
        }

        $('input[name=\'item-total[]\']').map(function() {
            if (!(Number($(this).val()) === parseFloat($(this).val()))) {
                e.preventDefault();
                $('#btn-simpan-single-outlet').prop('disabled', false);
                return alert("Jumlah harus berupa angka");
            }
        });
    });
    ...
    function resetElement() {
        elements = {
            ...
            tr: $("<tr></tr>"),
            td: $("<td></td>"),
            select: $("<select data-live-search=\"true\" data-placeholder=\"Pilih item...\"></select>").addClass('form-control').addClass('selectpicker').attr('name', 'item-name[]'),

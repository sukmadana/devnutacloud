                ...
                <div class="page-breadcrumb-wrap" style="width: 100%">
                    <div class="page-breadcrumb-info">
                        <form class="form-horizontal pull-right" method="get"
                              id="form-add" action="<?= base_url() ?>koreksistok/add">
                            <?php if (getLoggedInNamaPerusahaan() != "Individual") { ?>
                                <div class="form-group col-md-12">
                                    ...
                                    <div class="col-md-4">
                                        <input type="button"
                                               value="Tambah Koreksi Stok" <?php if ($visibilityMenu['StockAdd']) { ?>
...
<div class="row">
    <div class="col-md-12">
        <?php if ($selected_outlet): ?>
            ...
            <form action="<?= base_url("koreksistok/?outlet=" . $selected_outlet) ?>" method="post" id="filter-date"
                                ...
                                <table class="table table-bordered  table-striped " id="grid-item">
                                    ...
                                    <?php foreach ($list_stok as $key => $stok): ?>
                                        <tr>
                                            ...
                                            <!-- <td></td> -->
                                            <td>
                                                <?php if ($visibilityMenu['StockEdit'] && ($options->CreatedVersionCode>=98 || $options->EditedVersionCode>=98)) { ?>
                                                    <a href="<?php echo base_url("koreksistok/edit/" . $selected_outlet . "/" . $stok->TransactionID . "." . $stok->DeviceNo . "?ds=" . $date_start . "&de=" . $date_end); ?>"
                                                       class="btn btn-default">Edit</a>
                                                <?php } if ($options->CreatedVersionCode>=98 || $options->EditedVersionCode>=98) { ?>
                                                <a href="<?php echo base_url("koreksistok/view/" . $selected_outlet . "/" . $stok->TransactionID . "." . $stok->DeviceNo . "?ds=" . $date_start . "&de=" . $date_end); ?>"
                                                   class="btn btn-default">Lihat</a>
                                                <?php } if (1==0 && $visibilityMenu['StockDelete'] && ($options->CreatedVersionCode>=98 || $options->EditedVersionCode>=98)) { ?>
                                                    <button class="btn btn-default delete-button"
                                                            data-message="hapus <?= $stok->StockOpnameNumber ?>"
                                                            data-target=".delete-form-<?= $key ?>">Hapus
                                                    </button>
                                                <?php } ?>
                                                <form action="<?= base_url('koreksistok/destroy/' . urlencode($date_start) . "/" . urlencode($date_end)) ?>"
                                                      method="post" class="hidden delete-form-<?= $key ?>">
                                                    <input type="number" name="outlet_id"
                                                           value="<?= $selected_outlet ?>">
                                                    <input type="number" name="id" value="<?= $stok->TransactionID ?>">
                                                </form>
                                            </td>
                                        </tr>

            ...
            <div class="col-md-12">
                <div class="page-breadcrumb-wrap" style="width: 100%">
                    <div class="page-breadcrumb-info">
                        ...
                        <form class="form-horizontal pull-right" id="supplier-add" method="get"
                              action="<?= base_url() ?>supplier/form">
                        ...
                        <table class="table table-bordered  table-striped " id="grid-item">
                            ...
                            <tr>
                                <?php foreach ($datagrid['fields'] as $field) {
                                    ...
                                    if ($lowerfieldname == 'supplierid' || $lowerfieldname == 'deviceno') {
                                        continue;
                                    }
                                    if ($field->name == "Nama") {
                                        ?>
                                ...
                                <?php if ($visibilityMenu['SupplierEdit'] || $visibilityMenu['SupplierDelete']) { ?>
                                    <th></th>
                                <?php } ?>
                            </tr>
                            </thead>
                            ...
                            <tr>
                                ...
                                foreach ($datagrid['fields'] as $field) { ?>
                                    ...
                                    $fieldname = $field->name;
                                    $lowerfieldname = strtolower($fieldname);
                                    if ($lowerfieldname == 'suppliername') {
                                        continue;
                                    }
                                    if ($lowerfieldname == 'supplierid') {
                                        $id = $row->$fieldname;
                                ...
                                <?php if ($visibilityMenu['SupplierEdit'] || $visibilityMenu['SupplierDelete']) { ?>
                                    <td>
                                        <?php if ($visibilityMenu['SupplierEdit'] && ($options->CreatedVersionCode>=98 || $options->EditedVersionCode>=98)) { ?>
                                            <a href="<?= base_url('supplier/form?id=' . urlencode($id) . '&outlet=' . $selected_outlet. '&devno=' . $devno); ?>"
                                               class="btn btn-default">Edit</a>
                                        <?php }
                                        if ($visibilityMenu['SupplierDelete'] && ($options->CreatedVersionCode>=98 || $options->EditedVersionCode>=98)) { ?>
                                            <a class="btn btn-default delete-button"
                                               data-message="Hapus suplier ini? jika ya dihapus, jika tidak biarkan."
                                               data-target=".delete-form-<?= $row->SupplierID . "-" . $row->DeviceNo ?>">Hapus</a>
                                            <form action="<?php echo base_url(); ?>supplier/destroy" method="post"
                                                  class="hidden delete-form-<?= $row->SupplierID. "-" . $row->DeviceNo ?>">
                                                <input type="number" name="outlet" value="<?= $selected_outlet ?>">
                                                <input type="number" name="id" value="<?= $row->SupplierID ?>">

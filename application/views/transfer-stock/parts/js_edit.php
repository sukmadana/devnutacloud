<script type="text/javascript">
    ...
    $("select[name='to_outlet']").change(function(){
        //edit/2669/6?ds=2017-10-24&de=2017-10-24
        window.location.href = "<?php echo base_url(); ?>transferstok/edit/<?= $deviceid ?>/<?= $transfer->TransactionID ?>?ds=<?= $date_start ?>&de=<?= $date_end ?>&outlet_tujuan="+$(this).val();
        $(".widget-block").html("<center>Loading...</center>");
    });

    <?php if (isset($_SESSION['notif'])): ?>
    alert("<?= $_SESSION['notif'] ?>");
    window.location.href = "<?php echo base_url(); ?>transferstok?outlet=<?= $deviceid ?>";
    <?php endif ?>
</script>

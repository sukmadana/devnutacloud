<?php
    ...
    function printShift($dateStart, $timeStart, $dateEnd, $timeEnd, $result, $outlet, $ctx) {
        ...
        $listKategoriName = array();
        for ($i = 0; $i < count($result); $i++) {
            $om = $result[$i]->Kategori;
            if (!in_array($om, $listKategoriName)) {
                array_push($listKategoriName, $om);
            }
        }

        $grandTotalQty = 0;
        $grandTotalRp = 0;
        $rows = array_fill(0, count($listKategoriName), array("varianname" => "", "rowspan" => 0, "items" => array(), "subtotal_qty" => 0, "subtotal_rp" => 0));
        for ($i = 0; $i < count($listKategoriName); $i++) {
            $rows[$i]["varianname"] = $listKategoriName[$i];

            for ($j=0; $j < count($result); $j++) {
                if ($listKategoriName[$i] === $result[$j]->Kategori) {
                    $rows[$i]["rowspan"]++;
            ...
            <table class="table table-bordered table-striped">
                <thead>
                    <tr>
                        ...
                        <th>Total Qty per Kategori</th>
                        <th>Total Rp per Kategori</th>
                    </tr>      
                </thead>
                <tbody>
            <?php

...
function myFunction(obj)
{
    ...
    cityDropdown.attr('disabled','disabled');
    cityDropdown.append(jQuery('<option></option>').attr('value', '').text('Mohon Tunggu....'));
    if (province_id > 0) {
        const url = '<?=base_url('perusahaan/get_city_ajax/')?>' + '/' + province_id;
        jQuery.getJSON(url, function (data) {
            cityDropdown.empty();
            jQuery.each(data, function (key, entry) {
                if (entry.name == '<?=$detail_outlet->Kota;?>') {
                    cityDropdown.append(jQuery('<option></option>').attr('value', entry.name).text(entry.name));
                    cityDropdown.val(entry.name).trigger("change");
                } else {
                    cityDropdown.append(jQuery('<option></option>').attr('value', entry.name).text(entry.name));    
                }
            })

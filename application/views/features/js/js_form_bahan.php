...
?>
<script src="https://ws.nutacloud.com/js/item.js" type="text/javascript"></script>
<script type="text/javascript">
    ...
    function saveItemFinished(msg) {
        setTimeout(function () {
            alert(msg);
            window.location = '<?=base_url("bahan/index?outlet=" . $selected_outlet);?>';
    ...
    function goSavingItem(bunchOfPilihanEkstra, masterItemPostData) {
        ...
        if (bunchOfPilihanEkstra.length > 0) {
            console.log('with modifier');
            itemFormSaver.savePilihanEkstra('<?=base_url('ajax/savepilihanekstra');?>', {modifiers: bunchOfPilihanEkstra}).then(function (response) {
                var pOutlets = [];
                for (var x = 0; x < response.length; x++) {
                    var outlet = response[x].outlet;
                    console.log(" Simpan modifier di outlet: " + outlet);
                    var modifiers = response[x].modifiers;
                    for (var y = 0; y < modifiers.length; y++) {
                        console.log(' ==>Modifier: ' + modifiers[y].nama + ' tersimpan dengan id :' + modifiers[y].saved_id);
                    }
                    pOutlets.push(outlet);
                }
                sendToSocket(NEvent.TRANSFER_MODIFIER, {source: 'Save', outlets: pOutlets});
                sendToSocket(NEvent.TRANSFER_MODIFIER_DETAIL, {source: 'Save', outlets: pOutlets});

                return itemFormSaver.saveItem('<?=base_url('ajax/savemasteritem');?>', masterItemPostData);
                ...
                console.log('sendToSocket NEvent.TRANSFER_ITEM');
                sendToSocket(NEvent.TRANSFER_ITEM, {source: 'Save', outlets: outlets});
                console.log('sendToSocket NEvent.TRANSFER_MODIFIER_ITEM');
                sendToSocket(NEvent.TRANSFER_MODIFIER_ITEM, {source: 'Save', outlets: outlets});

                if (priceList.length > 0) {
                    console.log("Menyimpan variasi harga");
                    return new BulkDataSaver().saveVariasiHarga('<?=base_url();?>ajax/savevariasiharga', priceList, outlets, ids, arrayOfSavedItemID2);
                } else {
                    console.log("Tidak perlu menyimpan variasi harga");
                    return {ids: ids, outlets: outlets};
                }
            }).then(function (fromVariasiHarga) {
                sendToSocket(NEvent.TRANSFER_PRICE_VARIAN, {source: 'Save', outlets: fromVariasiHarga.outlets});
                if (hasImage()) {
                    var extension = $('#extfoto').val();
                    var imageItemPostData = new ImageItemPostData();
                    imageItemPostData.ids = fromVariasiHarga.ids;
                    imageItemPostData.ext = extension;
                    imageItemPostData.outlets = fromVariasiHarga.outlets;
            ...
            itemFormSaver.saveItem('<?=base_url('ajax/savemasteritem');?>', masterItemPostData).then(function (arrayOfSavedItemID) {
                var ids = [];
                var outlets = [];
                for (var x = 0; x < arrayOfSavedItemID.length; x++) {
                    var savedItem = arrayOfSavedItemID[x];
                ...
                if (priceList.length > 0) {
                    var vsSaver = new BulkDataSaver();
                    console.log("Saving variasi harga");
                    return vsSaver.saveVariasiHarga('<?=base_url();?>ajax/savevariasiharga', priceList, outlets, ids, arrayOfSavedItemID2);
                } else {
                    console.log("tidak perlu simpan variasi harga");
                    return {ids: ids, outlets: outlets};
                }
            }).then(function (fromVariasiHarga) {
                sendToSocket(NEvent.TRANSFER_PRICE_VARIAN, {source: 'Save', outlets: fromVariasiHarga.outlets});
    ...
    jQuery(document).ready(function ($) {
        ...
        $('#tambah-baris-bahan').click(function (e) {
            ...
            var barisbahan = '<tr class="itembahan">' +
                '<td><input type="text" class="form-control typeahead" data-tag="new" value=""/></td>' +
                '<td><input type="number" min="0" onkeypress="return ((event.charCode >= 48 && event.charCode <= 57)||(event.keyCode == 8 ||event.keyCode == 46))" class="form-control"/></td>' +
                '<td><input type="text" class="form-control typeaheadsatuan" data-tag="satuan" value="PCS"/></td>' +
...
//                console.log(response);
        ...
        $('#outlet').change(function () {
            window.location = '<?=$modeform == 'edit' ? base_url("bahan/itemform") . "?id=" . urlencode($form['nama item']) . "&outlet="
                    : base_url("bahan/itemform") . "?outlet=";?>' + $(this).val();
        });
                    ...
                    source: function (query, process) {
                        ...
                        var data = [
                            ...
                            foreach ($autocompletebahan as $row) {
                                $json .= "{ItemName:'" . str_replace("'","\'",$row->ItemName) . "',Satuan:'" . $row->Unit . "'},";
                            }
